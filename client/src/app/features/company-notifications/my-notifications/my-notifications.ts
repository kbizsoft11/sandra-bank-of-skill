...
@Component({
  selector: 'app-company-my-notifications',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './my-notifications.html',
  styleUrl: './my-notifications.scss',
})
export class CompanyMyNotifications implements OnInit {
  ...
  loadNotifications(): void {
    ...
    const endpoint = role === 'company'
      ? this.dashboardService.getCompanyNotificationsScalable(this.filter(), 1, 20)
  ...
  markAsReadFromModal(notification: EmployeeNotification): void {
    ...
    const role = this.authService.role();
    const endpoint = role === 'company'
      ? this.dashboardService.markCompanyNotificationAsReadScalable(notification.notificationId)
  ...
  getRolePrefix(): string {
    return this.authService.role() || 'company';
  }
