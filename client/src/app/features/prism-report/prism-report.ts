...
export class PrismReportComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private renderCharts(): void { this.charts.forEach((chart) => chart.destroy()); this.charts = []; this.createChart(this.behaviourChart, this.behaviourItems, 'PRISM profile value', '#5669d9'); this.createChart(this.aptitudeChart, this.aptitudeItems, 'PRISM profile value', '#e69a3a'); this.createChart(this.eiChart, this.eiItems.slice(0, 12), 'PRISM profile value', '#35a77a'); this.createChart(this.fourDChart, this.fourDData, 'PRISM profile value', '#7c3aed'); }
